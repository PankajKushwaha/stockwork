...
for stock in NIFTY500:
    data = yf.download(stock+".NS",period="10d",interval="15m",prepost = True)
    print(stock)
    print(data)
    if data is None:
        continue
    ...
    for ind in data.index:
        if "13:15" in str(ind):
            #print (data.loc[ind,"Open"])
            openPrice = data.loc[ind,"Open"]
            price_list.append(data.loc[ind,"Open"])
        if "15:15" in str(ind):
            #print (data.loc[ind,"Close"])
            closePrice = data.loc[ind,"Close"]
            price_list.append(data.loc[ind,"Close"])

    ma20 = round(sum(price_list)/20,2)
    stockPrice = nse.get_quote(stock)

    if stockPrice is None:
        continue

    #closePrice = stockPrice["closePrice"]
    #openPrice = stockPrice["open"]
    
    if  ((closePrice > ma20 and closePrice < (ma20 + ma20*margin)) and (closePrice < openPrice)) or ((closePrice < ma20 and closePrice > (ma20 - ma20*margin)) and (openPrice<closePrice)): 
        print(stock)
