...
for stock in NIFTY500:
    data = yf.download(stock+".NS",period="1d",interval="15m")
    if data is None:
        continue
    #print(stock)
    h=float(data.tail(8).head(1)["High"])
    l=float(data.tail(8).head(1)["Low"])
    o=float(data.tail(8).head(1)["Open"])
    c=float(data.tail(8).head(1)["Close"])
    #print(h)
    count=count+1
    print(count)

    if c > (o + (o/100) * 0.5) and c < (o+(o/100) * 0.8) :
        print("===")
        print(stock)
