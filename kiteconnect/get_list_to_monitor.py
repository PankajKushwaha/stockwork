...
def save_list_to_file_bull(list_name):
    with open("listtomonitor_bull", 'wb') as handle:
        pickle.dump(list_name, handle, protocol=pickle.HIGHEST_PROTOCOL)

def save_list_to_file_bear(list_name):
    with open("listtomonitor_bear", 'wb') as handle:
        pickle.dump(list_name, handle, protocol=pickle.HIGHEST_PROTOCOL)
...
def list_to_monitor(timeframe):
    list_to_monitor_bull=[]
    list_to_monitor_bear=[]
    ohlc=kite.ohlc(NIFTYFNO)
    for symbol in NIFTYFNO:
        print(symbol)
        date=datetime.today().strftime('%Y-%m-%d')
        candle = kite.historical_data(symbol_to_token[symbol], "2022-10-10 00:00:00", date+" 15:30:00",timeframe , continuous=False, oi=False)
        sma_list=[]
        for i in candle:
            '''
            o=i["open"]
        ...
        if sma20>sma40 and sma40>sma200 and c > sma20 :
            list_to_monitor_bull.append(symbol)
        if sma20<sma40 and sma40<sma200 and c < sma20 :
            list_to_monitor_bear.append(symbol)
    print(list_to_monitor_bull)
    print(list_to_monitor_bear)
    return list_to_monitor_bull,list_to_monitor_bear


while True:
    lst_bull_5min,lst_bear_5min = list_to_monitor("5minute")
    lst_bull_15min,lst_bear_15min = list_to_monitor("15minute")
    lst_bull=list(set(lst_bull_5min)&set(lst_bull_15min))
    lst_bear=list(set(lst_bear_5min)&set(lst_bear_15min))
    if os.path.isfile("/home/pankaj/Documents/stocks/kiteconnect/listtomonitor"):
        os.remove("/home/pankaj/Documents/stocks/kiteconnect/listtomonitor")
    save_list_to_file_bull(lst_bull)
    save_list_to_file_bear(lst_bear)
    time.sleep(300)
