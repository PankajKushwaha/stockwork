...
def save_screenshot(fnolst):
    ...
    for stock in fnolst:
        pyautogui.click(176,137)
        pyautogui.typewrite(stock)
        time.sleep(1)
        pyautogui.click(155,213)
        time.sleep(1)
        image = pyautogui.screenshot()
        image = cv2.cvtColor(np.array(image),
                     cv2.COLOR_RGB2BGR)
...
def write_str_to_file(filename,mystr):
...
def write_current_time_to_file():
    ...
    write_str_to_file("current_time.txt",formatted_time)
...
def get_1min_ago_time():
    time_str = read_time_from_file()
    minutes, seconds = map(int, time_str.split(":"))
    minutes -= 1
    if minutes < 0:
        minutes = 59  
    new_time_str = "{:02d}:{:02d}".format(minutes, seconds)
    return new_time_str 

def get_time_list():
    time_str = get_1min_ago_time()
    minutes, seconds = map(int, time_str.split(":"))
    time_list = []
    for sec in range(1, 60):
        time_list.append("{:02d}:{:02d}".format(minutes, sec))
    time_list = [s + ":" for s in time_list]
    return time_list

def contains_substring(s):
    timelst=get_time_list()
    return any(sub in s for sub in timelst)
