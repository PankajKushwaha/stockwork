...
def get_to_from_date(timeframe):
    if timeframe == "5minute":
        ...
        arg=sys.argv[1]
        dt = datetime.strptime(arg, '%Y-%m-%d')
        day_ago_400 = dt - relativedelta(days=400)
        to_date = dt - relativedelta(days=1)
        to_date=to_date.strftime('%Y-%m-%d')+" 15:35:00"
        from_date = day_ago_400.strftime('%Y-%m-%d') + " 09:15:00"
        return from_date,to_date

def is_ma_incline(symbol):
    from_date,to_date=get_to_from_date("day")
    candle = kite.historical_data(symbol_to_token[symbol], from_date, to_date,"day", continuous=False, oi=False)
    sma_list=[]
    for i in candle:
        c=i["close"]
        sma_list.append(c)
    sma20_1 = sum(sma_list[-20:])/20
    sma20_2 = sum(sma_list[-21:-1])/20
    sma20_3 = sum(sma_list[-22:-2])/20

    if sma20_1>sma20_2 and sma20_2>sma20_3:
        return True
    else:
        return False

def is_ma_decline(symbol):
    from_date,to_date=get_to_from_date("day")
    candle = kite.historical_data(symbol_to_token[symbol], from_date, to_date,"day", continuous=False, oi=False)
    sma_list=[]
    for i in candle:
...
def get_20_ma_till_date(symbol,timeframe):
    from_date,to_date=get_to_from_date(timeframe)
    candle = kite.historical_data(symbol_to_token[symbol], from_date, to_date,"day", continuous=False, oi=False)
    sma_list=[]
    for i in candle:
        c=i["close"]
...
def is_5minhigh_dayhigh(symbol):
    from_date,to_date=get_to_from_date("5minute")
    ...
    for i in candle:
        ...
        l=i["low"]
        highlist.append(h)
    if candle[0]["high"] == max(highlist):
        return True
    else:
        return False

def previous_candle_color(symbol):
    from_date,to_date=get_to_from_date("day")
    candle = kite.historical_data(symbol_to_token[symbol], from_date, to_date,"day", continuous=False, oi=False)
    o=candle[-1]["open"]
    c=candle[-1]["close"]
...
def is_5minlow_daylow(symbol):
    ...
    for i in candle:
        ...
        l=i["low"]
        lowlist.append(l)
    if candle[0]["low"] == min(lowlist):
        return True
    else:
        return False


def get_first_5min_ohlc_of_date(symbol):
    from_date,to_date=get_to_from_date("5minute")
    candle = kite.historical_data(symbol_to_token[symbol], from_date, to_date,"5minute", continuous=False, oi=False)
    for i in candle:
        c=i["close"]
...
def backtest_singl_symbol(symbol):
    ma20,ma40,ma200=get_20_ma_till_date(symbol,"day")
    o,h,l,c=get_first_5min_ohlc_of_date(symbol)

    #if l<ma20 and o>ma20 and c>ma20 and is_ma_incline(symbol) and is_5minhigh_dayhigh(symbol) == False and previous_candle_color(symbol)=="green" and ma20>ma40>ma200:
    #if l<ma20 and o>ma20 and c>ma20 and is_ma_incline(symbol) and is_5minhigh_dayhigh(symbol) == False and ma20>ma40>ma200:
    if l<ma20 and o>ma20 and c>ma20 and is_ma_incline(symbol) and ma20>ma40>ma200:
        print("long")
        print(symbol)
    #if h>ma20 and o<ma20 and c<ma20 and is_ma_decline(symbol) and is_5minlow_daylow(symbol) == False and ma20<ma40<ma200:
    if h>ma20 and o<ma20 and c<ma20 and is_ma_decline(symbol) and ma20<ma40<ma200:
        print("short")
        print(symbol)

for symbol in NIFTYFNO:
    backtest_singl_symbol(symbol)
