...
def list_to_monitor():
    ...
    for symbol in NIFTYFNO:
        candle = kite.historical_data(symbol_to_token[symbol], "2022-07-06 00:00:00", "2022-07-13 15:30:00", "5minute", continuous=False, oi=False)
        sma_list=[]
        print(symbol)
        for i in candle:
            '''
        ...
        if sma20<sma40<sma200 or sma20>sma40>sma200:
            print(sma20)
            print(sma200)
            min_ma=min(sma20,sma40,sma200)
            max_ma=max(sma20,sma40,sma200)
            per_change=(max_ma-min_ma)/min_ma
            range_dict[symbol]=per_change
    x=sorted(range_dict.items(), key=lambda item: item[1])
    pprint.pprint(x)
    as_list = [ seq[0] for seq in x ]
    print(as_list[:30])
    save_screenshot(as_list[:10])
