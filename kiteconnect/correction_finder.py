...
def bull_correction(lst):
    day_high=max(lst)
    lst_iter = iter(lst)
    if lst[0]>lst[1]:
        for i in revesed(lst):
...
def list_to_monitor():
    ...
    for symbol in NIFTYFNO:
        print(symbol)
        handle_time()
        candle = kite.historical_data(symbol_to_token[symbol], "2022-09-26 00:00:00", "2022-10-07 15:30:00", "5minute", continuous=False, oi=False)
        candle2 = kite.historical_data(symbol_to_token[symbol], "2022-10-07 00:00:00", "2022-10-07 15:30:00", "5minute", continuous=False, oi=False)
        sma_list=[]
        low_list=[]
        high_list=[]
        for i in candle:
