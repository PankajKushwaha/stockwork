...
def get_fno_symbol(symbol):
    current_month="MAY"
    current_year="24"
    symbol=symbol[4:]+current_year+current_month+"FUT"
    return symbol
...
lot=lot=get_lot_size()

def buy_fno_symbol(symbol):
    fnosymbol=get_fno_symbol(symbol)
    quantity = lot[symbol[4:]]

    kite.place_order(variety='regular', exchange='NFO', tradingsymbol=fnosymbol,
            transaction_type='BUY', quantity=quantity, product='NRML', order_type='MARKET', validity='DAY',
...
def sell_fno_symbol(symbol):
    fnosymbol=get_fno_symbol(symbol)
    quantity = lot[symbol[4:]]

    kite.place_order(variety='regular', exchange='NFO', tradingsymbol=fnosymbol,
            transaction_type='SELL', quantity=quantity, product='NRML', order_type='MARKET', validity='DAY',
            disclosed_quantity=None, trigger_price=None, squareoff=None, stoploss=None, trailing_stoploss=None, tag=None)
    os.system('spd-say "buy order executed"')
    exit()
