...
def save_to_file():
    ...
    stock_str = sys.argv[1]
    stock_list = stock_str.split(',')
    print(stock_list)
    print(stock_str)

    if os.path.exists("ma200_stocks.txt"):
        with open("ma200_stocks.txt", 'r+') as file:
            file_content = file.read()
            file_content = file_content.replace('\x00', '')
            total_str = file_content.rstrip()+","+stock_str
            print(file_content.rstrip()+","+stock_str)
            total_lst = set(total_str.split(','))
            print(total_lst)
            str_to_write = ','.join(total_lst)
            if str_to_write[1:] == ",":
                str_to_write = str_to_write[1:]
            print(str_to_write)
            with open("ma200_stocks.txt", 'w') as file:
                file.truncate(0)
    ...
    if os.path.exists("ma200_stocks.txt"):
        with open("ma200_stocks.txt", 'r+') as file:
            ...
            for symbol in watchlist:
                ...
                print(sma200)
                if l<=sma200<=h:
                    os.system('spd-say "stock between two hundred M A "')
                    print(symbol)

    # Open the file in write mode with the hardcoded filename
    '''
    filename = "ma20_stock.txt"
