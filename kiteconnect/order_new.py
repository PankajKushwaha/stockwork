...
def get_quantity(stock):
    ...
    return lot_size[stock.split("24")[0]]
...
def buy_fno_symbol(symbol,n):
    ...
    kite.place_order(variety='regular', exchange='NFO', tradingsymbol=symbol,
            transaction_type='BUY', quantity=quantity, product='NRML', order_type='LIMIT', price=c+3, validity='DAY',
...
def sell_fno_symbol(symbol,c,n):
    #c=ltp_fno_symbo(symbol)
    quantity = get_quantity(symbol)
    quantity= quantity*n
...
def order(symbol,condition,price,action,fno_symbol,ltp_dict,n):
    ...
    if condition == 'l':
        ...
        if c<price and action == "s":
            ltp_dic=get_open_position_price_dic(fno_symbol)
            c=ltp_fno_symbo(fno_symbol)
            sell_fno_symbol(fno_symbol,c,n)
...
def read_order():
    if os.path.isfile("/home/pankaj/Documents/stocks/kiteconnect/readorder.txt"):
        ...
        os.system("cat order_book.py|tail -1")
        os.execv(sys.executable, ['python3'] + sys.argv)
...
while True:
    ...
    #order("NSE:ACC","g",2541.95,"b","ACC24MAYFUT",ltpdict,1)
    #order("NSE:INDUSINDBK","l",1407.1,"s","INDUSINDBK24MAYFUT",ltpdict,1)
