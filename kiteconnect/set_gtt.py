...
def get_open_position_price_dic():
    ...
    for dic in pos['net']:
        ...
        symbol=dic['tradingsymbol']
        q=dic['buy_quantity']
        if q>0:
            position="bull"
        else:
            postion="bear"
        return symbol,position,q

def last_min5_hl(symbol):
    date=datetime.date.today().strftime('%Y-%m-%d')
    five_day_ago = datetime.date.today() - relativedelta(days=5)
    from_date = five_day_ago.strftime('%Y-%m-%d') + " 00:00:00"
    to_date = date+" 15:35:00"
    symbol="NFO:"+symbol
    ltpdic=kite.ltp(symbol)
    token=ltpdic[symbol]["instrument_token"]
    ltp=ltpdic[symbol]["last_price"]
    candles = kite.historical_data(token, from_date, date+" 15:35:00", "5minute", continuous=False, oi=False)
    h=candles[-1]["high"]
    l=candles[-1]["low"]
    return h,l,ltp

def place_gtt(symbol,position,quantity,h,l,ltp):
    if position == "bull":
        order_dict=[{"transaction_type": kite.TRANSACTION_TYPE_SELL, "quantity": quantity, 
                   'order_type': kite.ORDER_TYPE_LIMIT, "product": kite.PRODUCT_NRML , "price": l}]
        trigger_price=l
        if l>ltp-(ltp/400):
            trigger_price=ltp-(ltp/400)
        trigger_price=int(trigger_price*100)/100
        kite.place_gtt(kite.GTT_TYPE_SINGLE, symbol, kite.EXCHANGE_NFO, [trigger_price], ltp, order_dict)
    if position == "bear":
        order_dict=[{"transaction_type": kite.TRANSACTION_TYPE_BUY, "quantity": quantity, 
                   'order_type': kite.ORDER_TYPE_LIMIT, "product": kite.PRODUCT_NRML , "price": h}]
        if h<ltp+(ltp/400):
            trigger_price=ltp+(ltp/400)
        trigger_price=int(trigger_price*100)/100
        kite.place_gtt(kite.GTT_TYPE_SINGLE, symbol, kite.EXCHANGE_NFO, [trigger_price], ltp, order_dict)
...
symbol,position,quantity=get_open_position_price_dic()
h,l,ltp=last_min5_hl(symbol)
place_gtt(symbol,position,quantity,h,l,ltp)
