...
#order("NSE:SYNGENE","g",584.85,"b","SYNGENE23MAR600CE",ltp_dict,1)
#python3 bsf.py g syngene

currentMonth="APR"
symbol=sys.argv[2].upper()
ordertype=sys.argv[1]
futsymbol=symbol+"24"+currentMonth+"FUT"
print(futsymbol)
...
candle = kite.historical_data(symbol_to_token["NSE:"+symbol], from_date, date+" 15:35:00", "5minute", continuous=False, oi=False)
#print(candle[-2]["high"])
#print(candle[-2]["low"])
high=candle[-2]["high"]
low=candle[-2]["low"]
...
loss_per_share = high-low

share_1rs_loss = 1/(high-low)
lq= share_1rs_loss *risk
...
print(high)
print(low)
print(kite.ltp("NFO:"+futsymbol))

#order("NSE:SYNGENE","g",584.85,"b","SYNGENE23MAR600CE",ltp_dict,1)
#order("NSE:SYNGENE","g",690.85,"b","SYNGENE23MAYFUT",ltpdict,1)

if ordertype == "b":
    strToWrite="order(" + "\"NSE:" + symbol + "\"" + "," + "\"g\"" + "," + str(high) + ",\"b\""+ ",\"" + futsymbol + "\",ltpdict,"+str(lq)+")"
    print(strToWrite)

if ordertype == "s":
    strToWrite="order(" + "\"NSE:" + symbol + "\"" + "," + "\"l\"" + "," + str(low) + ",\"s\""+ ",\"" + futsymbol + "\",ltpdict,"+str(lq)+")"
    print(strToWrite)

with open("readorder_tmp.txt", "w") as text_file:
    text_file.write("    "+strToWrite+"\n")
    print("")

if len(sys.argv) ==  4 and sys.argv[3] == "r":
    os.system("rm readorder_tmp.txt")

confirm = input(f"Is price fine \n")
if confirm.lower() == "y":
    os.rename("readorder_tmp.txt", "readorder.txt")
