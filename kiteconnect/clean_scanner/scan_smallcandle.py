...
fno_list = read_text_file_to_list("smallcandle_watchlist.txt")
...
def get_last_ohlc(symbol):
    timeframe="5minute"
    date=datetime.today().strftime('%Y-%m-%d')
    to_date=date+" 15:35:00"
    from_date=get_n_day_ago_date(timeframe)
    candle = kite.historical_data(symbol_to_token["NSE:"+symbol], from_date, to_date,timeframe, continuous=False, oi=False)
    o=candle[-2]["open"]
    h=candle[-2]["high"]
    l=candle[-2]["low"]
    c=candle[-2]["close"]
