...
def preConditionBull(symbol):
    pdd=load_structure_from_file("pdd")
    o1=pdd[symbol][0][0]
    h1=pdd[symbol][0][1]
    l1=pdd[symbol][0][2]
    c1=pdd[symbol][0][3]
...
def read_text_file_to_list():
    try:
        with open("is_stock_bw_200ma.txt", 'r') as file:
            lines = [line.rstrip('\n') for line in file]
        return lines
...
def is_5min_ohlc_bw_200ma():
    return_list = []
    watchlist = read_text_file_to_list()
    ...
    for symbol in watchlist:
        ...
        l=candle[-1]["low"]
        if l<=sma200<=h:
            if h1< sma200 or l1> sma200:
                return_list.append(symbol)
    return return_list

def is_5min_ohlc_bw_20ma(watchlist):
    return_list = []
    print(watchlist)
    watchlist = list(set(watchlist))
    date=date=datetime.today().strftime('%Y-%m-%d')
...
def get_20ma_min5_nCandle_ago(candle,n):
    #date=date=datetime.today().strftime('%Y-%m-%d')
    #to_date=date+" 15:35:00"
    #from_date=get_n_day_ago_date("5minute")
    #candle = kite.historical_data(symbol_to_token[symbol], from_date, to_date,"5minute", continuous=False, oi=False)
...
def create_bull_wl():
    lst = []
    for symbol in NIFTYFNO:
        if symbol in global_bull_bear_wl:
            break
        date=date=datetime.today().strftime('%Y-%m-%d')
        to_date=date+" 15:35:00"
        from_date=get_n_day_ago_date("5minute")
        ...
        print(symbol)
        for i in range(9):
            o,h,l,c = get_ohlc_min5_nCandle_ago(candle,i+2)
            ma20 = get_20ma_min5_nCandle_ago(candle,i+1)
            print(i)
            if l<ma20:
                break
            if i==8:
    ...
    for symbol in NIFTYFNO:
        ...
        for i in range(9):
            ...
            if i==8:
                print("####### ADDING SYMBOL ########")
                print(symbol)
                bear_lst.append(symbol)
                break
        candle.clear()
    return bull_lst,bear_lst


def scan_short_long_trent():
    lst=[]
    bull_wl,bear_wl = create_bull_bear_wl()
    global_bull_bear_wl.extend(bull_wl+bear_wl)
    t=wait_for_5min()
    print("before sleep")
    print(t)
    time.sleep(t+2)
    print("after sleep")
    lst=is_5min_ohlc_bw_20ma(bull_wl+bear_wl+global_bull_bear_wl)
    for symbol in lst:
        if symbol in global_bull_bear_wl:
            global_bull_bear_wl.remove(symbol)
    return lst
...
def scan_long_trend():
    ...
    time.sleep(t+2)
    print("after sleep")
    lst=is_5min_ohlc_bw_20ma(wl)
    return lst

def scan_short_trend():
    lst=[]
    wl = create_bear_wl()
    t=wait_for_5min()
    print("before sleep")
...
def past_n_candle_notbw_200ma(candle,n):
    ...
    for i in range(n):
        o,h,l,c=get_ohlc_min5_nCandle_ago(candle,i+2)
        sma200=get_200ma_min5_nCandle_ago(candle,i+2)
        print("high is")
        print(h)
        print("low is")
        print(l)
...
def price_cross_200ma_min5():
    ...
    for symbol in lst:
        ...
        l=candle[-1]["low"]
        c=candle[-1]["close"]
        if l<sma200<h and symbol not in min5_200ma:
            if past_n_candle_notbw_200ma(candle,5):
                min5_200ma.append(symbol)
...
def is_ma_between_high_low(symbol,timeframe,o,h,l,c,ltp_dict,ohlc_dict):
    ...
    if firsttime:
        ...
        '''
        if c<o and preConditionBull(symbol) == True:
            pass
        else:
            return False
        '''
        ...
        print(symbol)
        print(sma20)
        print("number of red candle")
        print(redCandle[symbol])
        print("number of green candle")
        print(greenCandle[symbol])
        save_structure_to_file_wrapper("global_cross_ma20",global_cross_ma20)
        play_sound()
        if isTLthan("09:45"):
            time.sleep(1)
        ...
        global_cross_ma200.append(symbol)
        '''
        if (c<o and preConditionBull(symbol) == True) or (c>o and preConditionBear(symbol) == True):
            pass
        else:
            return False
        '''
...
def stock_crossed_ma():
    ...
    if firsttime==True:
        ...
        for symbol in NIFTYFNO:
            ...
            c=ltp_dict[symbol]["last_price"]
            if is_ma_between_high_low(symbol,"day",o,h,l,c,ltp_dict,ohlc_dict)==True:
...
def save_screenshot_stock(stock):
    #pyautogui.moveTo(184,45)
    #pyautogui.click(clicks=1)
    #pyautogui.moveTo(166,122)
    pyautogui.moveTo(180,138)
    ...
    cv2.imwrite("/home/pankaj/Pictures/"+"_"+timenow+"_"+stock+".png", image)
    #change_timeframe()
...
def main_function(watchlist):
    ...
    #final_list=new_high_with_20ma()+new_low_with_20ma()
    ...
    #create_bear_wl()
    #final_list=scan_long_trend()
    #final_list=scan_short_trend()
    #final_list=scan_short_long_trent()
    ...
    #final_list=new_day_high(NIFTYFNO)+new_day_low(NIFTYFNO)
    #final_list=is_5min_ohlc_bw_200ma()
    ...
    try:
        ...
        append_lst_to_file("cross_200ma.txt",final_list)
        #add_stock(final_list)
        #print("#############################################")
    except:
        exit()
