...
for symbol in NIFTYFNO:
    ...
    from_date = '2024-03-10 09:00:00'
    to_date = '2024-03-27 16:00:00'
    interval = '5minute'  
    ...
    #print(symbol)
    #print(df)
    for index, row in df[-75:].iterrows():
        lowList = list(df[0:index+1]['low'])
        highList = list(df[0:index+1]['high'])
        #ma20List = list(df[index-40:index]['ma20'])
        #ma200List = list(df[index-40:index]['ma200'])
        ma20List = list(df[-75:]['ma20'])
        ma200List = list(df[-75:]['ma200'])
        o=row['open']
        h=row['high']
        l=row['low']
        c=row['close']
        ma200 = ma200List[0]
        if o<ma200 and c<ma200 and h>ma200 and c<o:
            print(symbol)
        break
        #print(ma20List)
        #print(ma200List)
        std_dev = np.std(ma200List)
        stddev[symbol]=std_dev
        absolute_differences = []
        avg_distance = []
        for elem1, elem2 in zip(ma20List, ma200List):
            absolute_diff = elem1 - elem2
            avg_dis = ((max(elem1,elem2) - min(elem1,elem2))/min(elem1,elem2))*100
            absolute_differences.append(absolute_diff)
            avg_distance.append(avg_dis)

        #std_dev = np.std(absolute_differences)
        std_dev = np.std(ma20List)
        #print(avg_distance)
        average = sum(avg_distance) / len(avg_distance)
        #if std_dev<0.8 and average<0.2:
        '''
        if row['close'] < 1000:
            if std_dev<0.2 :
                print(symbol)
        ...
        #if (lowList[-1] >=lowList[-2] >= lowList[-3] >= lowList[-4] >= lowList[-5]) and row['high']<(row['low']+(row['low']*0.002)) and row['high'] > row['ma20'] and row['low'] < row['ma20'] :
        #if (lowList[-1] >=lowList[-2] >= lowList[-3] >= lowList[-4] >= lowList[-5]) and row['high']<(row['low']+(row['low']*0.002)) and row['high'] > row['ma20'] and row['low'] < row['ma20'] and (highList[-2]<ma20List[-2] and highList[-3]<ma20List[-3] and highList[-4]<ma20List[-4] and highList[-5]<ma20List[-5] and highList[-6]<ma20List[-6] and highList[-7]<ma20List[-7] and highList[-8]<ma20List[-8] and highList[-9]<ma20List[-9] and highList[-10]<ma20List[-10]) :
        #if ((row['close']-row['open'])/(row['high']-row['low']))<-0.9 and row['high']<(row['low']+(row['low']*0.002)) and row['high'] > row['ma20'] and row['low'] < row['ma20'] and (highList[-2]<ma20List[-2] and highList[-3]<ma20List[-3] and highList[-4]<ma20List[-4] and highList[-5]<ma20List[-5] and highList[-6]<ma20List[-6] and highList[-7]<ma20List[-7] and highList[-8]<ma20List[-8] and highList[-9]<ma20List[-9] and highList[-10]<ma20List[-10]) :
        if mx<mn+(mn*0.002):
            #print(row['date'])
            pass
            #print(lowList)
...
sorted_items = sorted(stddev.items(), key=lambda x: x[1])
