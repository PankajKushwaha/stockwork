...
def kite_get_today_ohlc(symbol):
    #symbol="NSE:"+symbol
    o=today_ohlc_dict[symbol]["ohlc"]["open"]
    h=today_ohlc_dict[symbol]["ohlc"]["high"]
    l=today_ohlc_dict[symbol]["ohlc"]["low"]
    c=today_ohlc_dict[symbol]["last_price"]
    return o,h,l,c
...
def global_data():
    ...
    print(fnolist)
    max_trade_per_day=4
    fnolistnse = ["NFO:"+s for s in fnolist] 
    today_ohlc_dict=kite.ohlc(fnolistnse)
    yesterday_ohlc_dict=load_from_file("yesterday_ohlc_dict.pkl")
    print(yesterday_ohlc_dict)
    return fnolistnse,today_ohlc_dict,yesterday_ohlc_dict,max_trade_per_day

def list_of_all_position():
    open_position_list = []
    pos = kite.positions()
    for dic in pos['net']:
        if dic['quantity'] > 0:
...
def placeGTT(symbol,quantity,sl_price,target_price ,ltp):
    ...
    #kite.place_gtt(kite.GTT_TYPE_OCO, symbol, kite.EXCHANGE_NFO, [sl_price,target_price], ltp, order_dict)

def buy_option(symbol):
    stock_bull=symbol
    stock_bull = stock_bull.replace('NFO:','')
    o,h,l,c = kite_get_today_ohlc(symbol)
    quantity=get_quantity(stock_bull)
    #print(quantity)
    kite.place_order(variety='regular', exchange='NFO', tradingsymbol=stock_bull,
            transaction_type='BUY', quantity=quantity, product='NRML', order_type='LIMIT', price=c+20, validity='DAY',
    ...
    '''
    target=c*4
    placeGTT(stock_bull,quantity,c-(c/10),target, c)

fnolistnse,today_ohlc_dict,yesterday_ohlc_dict,max_trade_per_day=global_data()
total_trade_taken_today=0

def stratagy():
    all_position = list_of_all_position()
    total_trade_taken_today=len(all_position)
    for symbol in fnolistnse:
        to,th,tl,tc = kite_get_today_ohlc(symbol)
        #print(to)
        yo,yh,yl,yc = kite_get_yesterday_ohlc(symbol)
        try:
            if tc>to and ((tc-to)/(th-tl))>0.95 and to>8 and to<ma20[symbol]+(ma20[symbol]/10) and tc>ma20[symbol]:
                print(symbol)
        except:
            continue
        '''
        quantity = get_quantity(symbol.replace('NFO:',''))
        margin = int(quantity*tc)

        if total_trade_taken_today>=max_trade_per_day:
            sys.exit()
        if yo!=yc and yl!=yh and tc>to and to<yh+3 and tc>yh and total_trade_taken_today<max_trade_per_day and margin<20000 and symbol not in all_position:
            try:
                print(symbol)
                print("##########BUY########")
                print(margin)
                buy_option(symbol)
                total_trade_taken_today=len(list_of_all_position())
            except:
                continue
        '''
...
def main_function():
    while True:
        stratagy()
        time.sleep(15)
        #if current time >9:22 then exit
ma20=load_from_file("ma20.pkl")
main_function()
