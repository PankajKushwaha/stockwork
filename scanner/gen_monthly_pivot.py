...
def get_month_cpr(stock):
    ...
    if len(history_data.loc[:,'High'].values) is 0:
        return None
    month_high = np.amax(history_data.loc[:,'High'].values)
    month_low =  np.amin(history_data.loc[:,'Low'].values)
    month_close = float(history_data.tail(1).loc[:,'Close'].values)

   # print(month_high)
   # print(month_low)
   # print(month_close)
    
    center_pivot =  fix_precesion((month_high + month_low + month_close) / 3)
    bottom_cpr = fix_precesion((month_high + month_low) / 2)
    ...
    S1 =  fix_precesion((2 * center_pivot) - month_high)
    S2 =  fix_precesion(center_pivot - (month_high - month_low))
    R1 =  fix_precesion((2 * center_pivot) - month_low)
    R2 =  fix_precesion(center_pivot + (month_high - month_low))
...
pivot_df.to_csv('monthly_pivot_nifty500_JUN.csv')
