...
histor_data = get_history(symbol='SBIN',
                   start=date(2015,1,1),
                   end=date(2015,1,10))
histor_data.index.name=None
#print((history.loc[:,'High']).to_string(index=False))
#print(np.amax(history.loc[:,'High'].values))
...
def get_month_cpr(stock):
    histor_data = get_history(symbol=stock,
           start=date(2021,4,26),
          end=date(2021,4,30))
    histor_data.index.name=None
    if len(histor_data.loc[:,"High"].value_counts()) is 0:
        return None
    week_high = np.amax(histor_data.loc[:,'High'].values)
    week_low =  np.amin(histor_data.loc[:,'Low'].values)
    week_close = float((histor_data.tail(1).loc[:,'Close'].values))
    
    center_pivot =  fix_precesion((week_high + week_low + week_close) / 3)
    bottom_cpr = fix_precesion((week_high + week_low) / 2)
    top_cpr = fix_precesion((center_pivot - bottom_cpr ) + center_pivot)

    tmp = min(bottom_cpr, top_cpr)
    top_cpr = max(bottom_cpr,top_cpr)
    bottom_cpr = tmp
...
#   print(top_cpr)
    ...
    S1 =  fix_precesion((2 * center_pivot) - week_high)
    S2 =  fix_precesion(center_pivot - (week_high - week_low))
    R1 =  fix_precesion((2 * center_pivot) - week_low)
    R2 =  fix_precesion(center_pivot + (week_high - week_low))
...
#    print(R2)
    ...
    #print(week_close)
    #print(float(((history.tail(1)).loc[:,'Close']).values))
    #print(week_high)
    #print(week_low)
    #return history
...
pivot_df.to_csv('weekly_pivot_nifty500.csv')
