...
#    if close > (openPrice+(openPrice/100)*0.15) and high > low and ((high - low ) / (close - openPrice)) > 5 :
    if (close-openPrice) >0:
        if (high - low ) / (close - openPrice) > 5 and close>(openPrice+(openPrice/100)*0.3):
            print("========")
            print(stock)
            green_hammer_list.append(stock)
    
print(green_hammer_list)

for stock in green_hammer_list:
